...
- While a response is streaming, regular composer sends must remain available: empty composer shows stop, non-empty composer shows send. The default regular-send behavior is user configurable in Settings: steer, interrupt, or queue. Queue, interrupt, and steering failure/fallback notices remain pinned above the composer while the active response streams, then become transcript notices when the response completes. The successful-steer confirmation (`Steering hint delivered.`) is the only transient exception: show at most one above the composer, replace it and restart its 3-second dismissal timer after each later successful steering hint, and discard it if the response completes first; never persist it or add it to the transcript. This exception applies to both `/steer` and regular composer sends configured to steer.
