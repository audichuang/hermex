...
extension CustomHeader {
    ...
    ///
    /// `Authorization` is deliberately NOT on this list. Carrying it is the
    /// feature's main purpose — a reverse proxy or a token-auth deployment
    /// needs it, and the editor's own help text names it. The app authenticates
    /// with a session cookie and never sets `Authorization` itself, so a user
    /// value cannot clobber anything the app relies on.
