...
enum APIError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .http(let statusCode, let body):
            ...
            case 408:
                return String(localized: "The server took too long to respond. Check that the server host is awake and the server is running.")
            case 429:
            ...
            case 502, 503, 504:
                return String(localized: "The server or tunnel is unavailable. Check that the server host is awake, hermes-webui is running, and the tunnel is connected.")
            default:
                if let message = Self.serverErrorMessage(from: body) {
...
private extension APIError {
    ...
    static func networkMessage(for error: Error) -> String {
        ...
        case .timedOut:
            return String(localized: "The server did not respond in time. Check that the server host is awake, hermes-webui is running, and the connection is available.")
