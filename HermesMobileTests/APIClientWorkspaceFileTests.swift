...
final class APIClientWorkspaceFileTests: APIClientTestCase {
    ...
    @MainActor
    func testOfficeDocumentExportFetchesRawBytesAfterLoadingTextPreview() async throws {
        let rawData = Data([0x50, 0x4B, 0x03, 0x04, 0x14, 0x00])
        var requestedPaths: [String] = []
        let client = makeClient { request in
            let requestPath = request.url?.path ?? "nil"
            requestedPaths.append(requestPath)
            ...
            case "/api/file/raw":
                let response = HTTPURLResponse(
                    ...
                    statusCode: 200,
                    httpVersion: nil,
                    headerFields: ["Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document"]
                )
                return (try XCTUnwrap(response), rawData)
            default:
                XCTFail("Unexpected request path: \(requestPath)")
        ...
        guard case .text(let file) = viewModel.preview else {
            return XCTFail("Office document should retain its extracted-text preview.")
        }
        XCTAssertEqual(file.content, "Extracted Office text")
        XCTAssertEqual(payload.data, rawData)
        XCTAssertEqual(payload.filename, "spec.docx")
        XCTAssertEqual(requestedPaths, ["/api/file", "/api/file/raw"])
    }

    @MainActor
    func testOfficeDocumentExportKeepsActionableRawFileError() async throws {
        let serverHint = "Unable to read the original DOCX file."
        let client = makeClient { request in
            let response = HTTPURLResponse(
                url: try XCTUnwrap(request.url),
                statusCode: 404,
            ...
            return (try XCTUnwrap(response), Data(#"{"error":"\#(serverHint)"}"#.utf8))
        }
        let viewModel = try FilePreviewViewModel(
            session: makeFilePreviewSession(),
            server: XCTUnwrap(URL(string: "https://example.test")),
        ...
        do {
            _ = try await viewModel.exportPayload()
            XCTFail("Expected export to fail.")
        } catch {
            XCTAssertEqual(viewModel.exportErrorMessage, serverHint)
        }
    }

    @MainActor
    func testOfficePreviewShowsActionableServerMessageForJSON503() async throws {
        let dependencyHint = "Office preview is not available on this server. Install python-docx, openpyxl, and python-pptx to enable it."
        let client = makeClient { request in
            let response = HTTPURLResponse(
                url: try XCTUnwrap(request.url),
                statusCode: 503,
            ...
            return (
                ...
                Data(#"{"error":"\#(dependencyHint)"}"#.utf8)
            )
        }
        let viewModel = try FilePreviewViewModel(
            session: makeFilePreviewSession(),
        ...
        XCTAssertEqual(viewModel.errorMessage, dependencyHint)
    }

    @MainActor
    func testOfficePreviewShowsActionableServerMessageForJSON404() async throws {
        let serverHint = "Unable to read DOCX preview. The file may be invalid or corrupted."
        let client = makeClient { request in
            let response = HTTPURLResponse(
                url: try XCTUnwrap(request.url),
                statusCode: 404,
        ...
        XCTAssertEqual(viewModel.errorMessage, serverHint)
    }

    @MainActor
    func testOfficePreviewKeepsFriendlyVanishedSessionMessage() async throws {
        let client = makeClient { request in
            let response = HTTPURLResponse(
                url: try XCTUnwrap(request.url),
                statusCode: 404,
            ...
            return (try XCTUnwrap(response), Data(#"{"error":"Session not found"}"#.utf8))
        }
        let viewModel = try FilePreviewViewModel(
            session: makeFilePreviewSession(),
            server: XCTUnwrap(URL(string: "https://example.test")),
    ...
    @MainActor
    func testOfficePreviewKeepsGenericUnavailableMessageForProxy503() async throws {
        let client = makeClient { request in
            let response = HTTPURLResponse(
                url: try XCTUnwrap(request.url),
                statusCode: 503,
            ...
            return (try XCTUnwrap(response), Data("<html>proxy unavailable</html>".utf8))
        }
        let viewModel = try FilePreviewViewModel(
            session: makeFilePreviewSession(),
            server: XCTUnwrap(URL(string: "https://example.test")),
