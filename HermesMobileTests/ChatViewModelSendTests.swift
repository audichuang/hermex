...
final class ChatViewModelSendTests: XCTestCase {
    ...
    func testSubmitGoalAttachesToServerStartedKickoffStream() async throws {
                    ...
                    return apiTestJSONResponse("""
                    {
                      "session": {
                        ...
                        "runtime_journal_snapshot": {
                          ...
                          "last_seq": 3,
                          "last_event_id": "stream-goal:3",
                          "last_assistant_text": "Started. "
                        }
                      }
                    }
                    """, for: request)
        ...
        XCTAssertEqual(requestedPaths, ["/api/goal", "/api/session", "/api/session"])
        ...
        let streamQuery = Dictionary(uniqueKeysWithValues: (
            URLComponents(url: try XCTUnwrap(streamClient.startedURLs.first), resolvingAgainstBaseURL: false)?.queryItems ?? []
        ).map { ($0.name, $0.value ?? "") })
        XCTAssertEqual(streamQuery["after_seq"], "3")
        XCTAssertEqual(streamQuery["after_event_id"], "stream-goal:3")
        XCTAssertEqual(viewModel.messages.map(\.role), ["user", "assistant"])
        XCTAssertEqual(viewModel.messages.last?.content, "Started. ")
        XCTAssertEqual(viewModel.pinnedLocalNotices, ["Goal set."])

        streamClient.emit(.token("Working now."))

        XCTAssertEqual(viewModel.messages.map(\.role), ["user", "assistant"])
        XCTAssertEqual(viewModel.messages.last?.content, "Started. Working now.")
    }
    ...
    /// choice, so the pending card must remain actionable (#5).
    @MainActor
    ...
    func testReopeningActiveStreamRestoresLiveSnapshotBeforeBufferedTailArrives() async throws {
        ...
        XCTAssertTrue(didRequestStatus)
        // A same-run local snapshot already owns both visible state and cursor,
        // so neither load needs a redundant metadata-only journal request.
        XCTAssertEqual(sessionMessageModes, ["1", "1"])
    ...
    @MainActor
    func testCompletedResponseKeepsSidebarTitleWhenRefreshOnlyHasGeneratedPlaceholder() async throws {
        let streamClient = SpySSEStreamingClient()
        let summary = try makeSession(title: "Hermes WebUI #7")
        let viewModel = try makeViewModel(
            streamClient: streamClient,
            ...
            case "/api/session":
                return apiTestJSONResponse(
                    #"{"session":{"session_id":"session-abc","title":"Hermes WebUI #7"}}"#,
                    for: request
                )
            default:
                XCTFail("Unexpected request path: \(request.url?.path ?? "nil")")
        ...
        let didStart = await viewModel.sendMessage("Keep this title")
        XCTAssertTrue(didStart)
        streamClient.emit(.done(DoneStreamEvent()))
        streamClient.emit(.streamEnd)
        try await waitUntil { viewModel.activeStreamID == nil }

        XCTAssertEqual(viewModel.displayTitle, "Fix the flaky test")
    }

    @MainActor
    func testLoadKeepsSidebarDisplayTitleWhenDetailOnlyHasGeneratedPlaceholder() async throws {
        let summary = try makeSession(title: "Hermes WebUI #7")
        let viewModel = try makeViewModel(
            sessionSummary: SessionSummary(
                sessionId: summary.sessionId,
            ...
            XCTAssertEqual(request.url?.path, "/api/session")
            return apiTestJSONResponse(
                #"{"session":{"session_id":"session-abc","title":"Hermes WebUI #7","messages":[]}}"#,
                for: request
            )
        }
    ...
    @MainActor
    func testColdAttachMaterializesDeferredUserAndServerJournalSnapshotBeforeReplayingItsTail() async throws {
        let streamClient = SpySSEStreamingClient()
        var sessionQueries: [[String: String]] = []
        var viewModel: ChatViewModel!
        viewModel = try makeViewModel(streamClient: streamClient) { request in
            switch request.url?.path {
            case "/api/session":
                let components = URLComponents(url: try XCTUnwrap(request.url), resolvingAgainstBaseURL: false)
                let query = Dictionary(uniqueKeysWithValues: (components?.queryItems ?? []).map { ($0.name, $0.value ?? "") })
                sessionQueries.append(query)

                if query["messages"] == "0" {
                    XCTAssertEqual(
                        viewModel.messages.compactMap(\.content),
                        ["Keep working", "Old answer", "Keep working"]
                    )
                    return apiTestJSONResponse("""
                    {
                      "session": {
                        "session_id": "session-abc",
                        "active_stream_id": "stream-elsewhere",
                        "runtime_journal_snapshot": {
                          "session_id": "session-abc",
                          "stream_id": "stream-elsewhere",
                          "last_seq": 12,
                          "last_event_id": "run-stable:12",
                          "last_assistant_text": "Snapshot answer. ",
                          "last_reasoning_text": "Snapshot reasoning.",
                          "tool_calls": [
                            "malformed-row",
                            {
                              "tid": "tool-1",
        ...
        XCTAssertEqual(sessionQueries.count, 2)
        XCTAssertEqual(sessionQueries[0]["messages"], "1")
        XCTAssertEqual(sessionQueries[0]["msg_limit"], "20")
        XCTAssertEqual(sessionQueries[1]["messages"], "0")
        XCTAssertNil(sessionQueries[1]["msg_limit"])
        XCTAssertEqual(
            viewModel.messages.compactMap(\.content),
            ["Keep working", "Old answer", "Keep working", "Snapshot answer. "]
        )
        XCTAssertEqual(viewModel.liveReasoningText, "Snapshot reasoning.")
        XCTAssertEqual(viewModel.liveToolCalls.map(\.name), ["read_file"])
        XCTAssertEqual(viewModel.liveToolCalls.first?.isCompleted, true)

        let streamURL = try XCTUnwrap(streamClient.startedURLs.first)
        let streamQuery = Dictionary(uniqueKeysWithValues: (
            URLComponents(url: streamURL, resolvingAgainstBaseURL: false)?.queryItems ?? []
        ).map { ($0.name, $0.value ?? "") })
        XCTAssertEqual(streamQuery["stream_id"], "stream-elsewhere")
        XCTAssertEqual(streamQuery["replay"], "1")
        XCTAssertEqual(streamQuery["after_seq"], "12")
        XCTAssertNil(streamQuery["after_event_id"])

        streamClient.emit(.token("Tail."), lastEventID: "stream-elsewhere:13")

        XCTAssertEqual(
            viewModel.messages.compactMap(\.content),
            ["Keep working", "Old answer", "Keep working", "Snapshot answer. Tail."]
        )
    }

    @MainActor
    func testColdAttachDoesNotDuplicateCachedOptimisticPendingUserMessage() async throws {
        let context = try makeContext()
        let serverURL = URL(string: "https://example.test")!
        try CacheStore.cacheMessages(
            [
        ...
        let viewModel = try makeViewModel { request in
            ...
            if query.contains(where: { $0.name == "messages" && $0.value == "0" }) {
                return apiTestJSONResponse("""
                {"session":{"session_id":"session-abc","active_stream_id":"stream-elsewhere",
                 "runtime_journal_snapshot":{"stream_id":"stream-elsewhere","last_seq":4,
                 "last_assistant_text":"Snapshot answer."}}}
                """, for: request)
            }
            ...
            return apiTestJSONResponse("""
            {"session":{"session_id":"session-abc","active_stream_id":"stream-elsewhere",
             "pending_user_message":"Keep working","pending_started_at":1770000100,
             "messages":[
               {"role":"user","content":"Earlier question","message_id":"user-old"},
               {"role":"assistant","content":"Old answer","message_id":"assistant-old"}
             ]}}
            """, for: request)
        }
        ...
        XCTAssertEqual(viewModel.messages.compactMap(\.content), [
            "Earlier question", "Old answer", "Keep working", "Snapshot answer."
        ])
        XCTAssertEqual(viewModel.messages.filter { $0.role == "user" && $0.content == "Keep working" }.count, 1)
    }

    @MainActor
    func testColdAttachPreservesParallelSnapshotToolsAndErrorState() async throws {
        let viewModel = try makeViewModel { request in
            let query = URLComponents(url: try XCTUnwrap(request.url), resolvingAgainstBaseURL: false)?.queryItems ?? []
            if query.contains(where: { $0.name == "messages" && $0.value == "0" }) {
                return apiTestJSONResponse("""
                {"session":{"session_id":"session-abc","active_stream_id":"stream-tools",
                 "runtime_journal_snapshot":{"stream_id":"stream-tools","last_seq":9,
                 "last_assistant_text":"Checking both files.","tool_calls":[
                   {"tid":"tool-a","name":"read_file","snippet":"First","done":false,"is_error":false},
                   {"tid":"tool-b","name":"read_file","snippet":"Second","done":true,"is_error":true}
                 ]}}}
                """, for: request)
            }
            ...
            return apiTestJSONResponse("""
            {"session":{"session_id":"session-abc","active_stream_id":"stream-tools",
             "messages":[{"role":"user","content":"Read both files","message_id":"user-tools"}]}}
            """, for: request)
        }

        await viewModel.loadMessages()

        XCTAssertEqual(viewModel.liveToolCalls.map(\.id), ["tool-a", "tool-b"])
        XCTAssertEqual(viewModel.liveToolCalls.map(\.name), ["read_file", "read_file"])
        XCTAssertEqual(viewModel.liveToolCalls.map(\.isCompleted), [false, true])
        XCTAssertNil(viewModel.liveToolCalls[0].isError)
        XCTAssertEqual(viewModel.liveToolCalls[1].isError, true)
    }

    @MainActor
    func testColdAttachWithEmptyJournalSnapshotDoesNotReplayFromZero() async throws {
        let streamClient = SpySSEStreamingClient()
        var sessionMessageModes: [String] = []
        let viewModel = try makeViewModel(streamClient: streamClient) { request in
            switch request.url?.path {
            case "/api/session":
                ...
                return apiTestJSONResponse("""
                {
                  "session": {
                    ...
                    "active_stream_id": "stream-empty",
                    "messages": [
                      {"role":"user","content":"Waiting","timestamp":1770000200,"message_id":"user-2"}
                    ]
                  }
                }
                """, for: request)
            case "/api/chat/stream/status":
                return apiTestJSONResponse(
                    #"{"active":true,"stream_id":"stream-empty","replay_available":true}"#,
                    for: request
                )
            default:
                XCTFail("Unexpected request path: \(request.url?.path ?? "nil")")
        ...
        XCTAssertEqual(sessionMessageModes, ["1", "0", "1", "0"])
        XCTAssertEqual(viewModel.messages.compactMap(\.content), ["Waiting"])
        let streamURL = try XCTUnwrap(streamClient.startedURLs.first)
        let streamQuery = Dictionary(uniqueKeysWithValues: (
            URLComponents(url: streamURL, resolvingAgainstBaseURL: false)?.queryItems ?? []
        ).map { ($0.name, $0.value ?? "") })
        XCTAssertNil(streamQuery["replay"])
        XCTAssertNil(streamQuery["after_seq"])
        XCTAssertNil(streamQuery["after_event_id"])
    }

    @MainActor
    func testColdAttachFallsBackToSnapshotSequenceWhenEventIDIsMalformed() async throws {
        let streamClient = SpySSEStreamingClient()
        let viewModel = try makeViewModel(streamClient: streamClient) { request in
            switch request.url?.path {
            case "/api/session":
                ...
                if query["messages"] == "0" {
                    return apiTestJSONResponse("""
                    {
                      "session": {
                        "session_id": "session-abc",
                        "active_stream_id": "stream-sequence",
                        "runtime_journal_snapshot": {
                          "session_id": "session-abc",
                          "stream_id": "stream-sequence",
                          "last_seq": 7,
                          "last_event_id": "malformed",
                          "last_assistant_text": "Recovered from the journal."
                        }
                      }
                    }
                    """, for: request)
                ...
                return apiTestJSONResponse("""
                {
                  "session": {
                    ...
                    "active_stream_id": "stream-sequence",
                    "messages": [
                      {"role":"user","content":"Continue","timestamp":1770000300,"message_id":"user-3"}
                    ]
                  }
                }
                """, for: request)
            case "/api/chat/stream/status":
                return apiTestJSONResponse(
                    #"{"active":true,"stream_id":"stream-sequence","replay_available":true}"#,
                    for: request
                )
            default:
                XCTFail("Unexpected request path: \(request.url?.path ?? "nil")")
        ...
        XCTAssertEqual(viewModel.messages.compactMap(\.content), ["Continue", "Recovered from the journal."])
        let streamURL = try XCTUnwrap(streamClient.startedURLs.first)
        let streamQuery = Dictionary(uniqueKeysWithValues: (
            URLComponents(url: streamURL, resolvingAgainstBaseURL: false)?.queryItems ?? []
        ).map { ($0.name, $0.value ?? "") })
        XCTAssertEqual(streamQuery["replay"], "1")
        XCTAssertEqual(streamQuery["after_seq"], "7")
        XCTAssertNil(streamQuery["after_event_id"])
    }
